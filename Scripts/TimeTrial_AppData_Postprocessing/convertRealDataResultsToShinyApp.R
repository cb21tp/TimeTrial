...
RAIN_fileNames <- list.files(pattern="*.txt")
getRid <- list.files(pattern="RAIN_Results_Adjusted_*")
RAIN_fileNames <- setdiff(RAIN_fileNames, getRid)
...
results <- list(Arser,BooteJTK,JTK_Cycle,RAIN)
rm(RAIN,Arser,BooteJTK,JTK_Cycle)

save.image("~/Desktop/TimeTrial/TimeTrial_Apps/App_Data/timeTrialRealDataComplete.RData")
