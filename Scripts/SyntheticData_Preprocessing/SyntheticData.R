...
saw <- function(tp, amp,shift,asym){
  up <- seq(from = 0, to = amp, length.out = (round(asym*period)/2))
  down <- rev(seq(from = 0, to = amp, length.out = (period-round(asym*period)+3)/2))
  output <- c(rep(c(up, down[-c(1,length(down))]), 2*length(tp)/period),down[length(down)])
  shift <- shift %% 24
...
sawRand <- sample(c(.2,.3,.8,.9), size = nsamples, replace = T) #saw tooth shape
...
#Name Samples
waveForms <- c("sin", "peak", "saw","lTrend", "damp", "amped", "contract", "flat", "linear", "sigmoid", "decay")
